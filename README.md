...
The server must run a compatible version of [Northwood's LabAPI](https://github.com/northwood-studios/LabAPI). The project targets `net48` and uses `Northwood.LabAPI` version `1.1.7`.

A build machine also needs the matching SCP: Secret Laboratory dedicated-server managed assemblies. The project references these assemblies through the `SL_REFERENCES` and `UNITY_REFERENCES` MSBuild properties.
